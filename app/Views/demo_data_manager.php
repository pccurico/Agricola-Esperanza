...
$activeInstalledAt = $active['installed_at'] ?? null;
$activeInstallationId = $active['installation_id'] ?? null;
$activeStatus = $active['status'] ?? null;
$activeRecordsCount = (int) ($active['records_count'] ?? 0);
?>
<!doctype html>
<html lang="es">
<head>
        ...
        <section class="module-content">
            ...
            <section class="admin-columns">
                <article class="admin-panel">
                    ...
                    <?php if ($active): ?>
                        <div class="role-list"><div class="role-row"><div><b>Conjunto instalado</b><small>Instalado el <?= htmlspecialchars((string) ($activeInstalledAt ?? '—'), ENT_QUOTES, 'UTF-8') ?></small></div><span><?= $activeRecordsCount ?> registros</span></div><div class="role-row"><div><b>Identificador</b><small><?= htmlspecialchars((string) ($activeInstallationId ?? '—'), ENT_QUOTES, 'UTF-8') ?></small></div><span><?= htmlspecialchars((string) ($activeStatus ?? '—'), ENT_QUOTES, 'UTF-8') ?></span></div></div>
            ...
            <section class="admin-panel"><header class="panel-header"><h2>Historial de instalaciones</h2><p>Consulta cuándo se cargaron o retiraron conjuntos demo.</p></header><div class="table-scroll"><table class="admin-table"><thead><tr><th>Identificador</th><th>Versión</th><th>Estado</th><th>Instalado</th><th>Retirado</th></tr></thead><tbody><?php foreach ($history as $batch): ?><tr><td><?= htmlspecialchars((string) ($batch['installation_id'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) ($batch['version'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) ($batch['status'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) ($batch['installed_at'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td><td><?= htmlspecialchars((string) ($batch['removed_at'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></td></tr><?php endforeach; ?></tbody></table></div></section>
        </section>
    </main>
</body>
</html>
