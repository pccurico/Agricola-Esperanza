            ...
            <section class="section-card">
                <div class="section-head">
                    ...
                    <div class="header-actions">
                        <a class="primary-button" href="<?= htmlspecialchars(module_url('users', ['new_user' => 1]), ENT_QUOTES, 'UTF-8') ?>">Crear usuario</a>
                        ...
                        <label>Rol
                            <select name="role_id" required>
                                <?php foreach ($roles as $role): ?>
                                    <option value="<?= (int) $role['id'] ?>" <?= ((int) ($selected_user['role_id'] ?? 0) === (int) $role['id']) ? 'selected' : '' ?>><?= htmlspecialchars($role['name'], ENT_QUOTES, 'UTF-8') ?></option>
                                <?php endforeach; ?>
                            </select>
                        </label>
                        <label>Contraseña
                            <input type="password" name="password" <?= $selected_user ? '' : 'required minlength="10"' ?>>
                        </label>
                        <div class="form-actions">
                            <button class="primary-button" type="submit"><?= $selected_user ? 'Guardar usuario' : 'Crear usuario' ?></button>
