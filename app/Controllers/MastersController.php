...
use AgroPCC\Services\MasterData;

final class MastersController extends BaseController
{
    public function handle(): array
    {
        $manager = new \AgroPCC\Services\MasterData(database()->connection(), (int) $_SESSION['company_id']);
        $error = null;
        $success = null;
        try {
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_master') {
                $manager->create((string) ($_POST['type'] ?? ''), $_POST);
                (new \AgroPCC\Services\AuditLog(database()->connection(), (int) $_SESSION['company_id']))->record((int) $_SESSION['user_id'], 'CREATE', 'master', null, ['type' => $_POST['type'] ?? null]);
