...
final class MasterData
{
    ...
    private function createCostCenter(array $input): void
    {
        ...
        $categories = ['ADMINISTRACION', 'MANO_DE_OBRA', 'INVERSION', 'SERVICIOS_GASTOS', 'BODEGA'];
        if (!in_array($input['category'], $categories, true)) {
            throw new RuntimeException('La categoría del centro de costo no es válida.');
        }
        $this->execute('INSERT INTO cost_centers (company_id, code, name, category) VALUES (?, ?, ?, ?)', [$this->companyId, strtoupper(trim($input['code'])), trim($input['name']), $input['category']]);
    }
